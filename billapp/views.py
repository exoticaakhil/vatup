...
def get_partydetails(request):
    if request.method == 'POST':
        ...
        sales_invoice = Invoice.objects.filter(company=cmp, party=party).first()
        ...
        placeofsupply = sales_invoice.party.address if sales_invoice else None
        payment = sales_invoice.party.payment if sales_invoice else None
...
def get_invoice_date(request):
    ...
    try:
        invoice = Invoice.objects.get(invoice_no=invoiceno)  # Assuming 'invoice_no' is the field name in your Invoice model
        invoicedate = invoice.invoice_date  # Assuming 'date' is the field name in your Invoice model
        print(invoicedate,'date')
...
def get_invoice_item(request):
    ...
    try:
        ...
        invoice = get_object_or_404(Invoice, invoice_no=invoiceno)
        invoice_items = InvoiceItem.objects.filter(company=cmp, salesinvoice=invoice)
    except Invoice.DoesNotExist:
        return redirect(SalesReturn)
...
def updateCreditnote(request,pk):
  if request.method=="POST":
    ...
    if creditnote.partystatus=='partyon':
      ...
      if salesinvoice:
        idsales=request.POST['invoiceno']
        creditnote.salesinvoice=Invoice.objects.get(invoice_no=idsales,company=cmp)
        creditnote.save()
      else:
        pass
